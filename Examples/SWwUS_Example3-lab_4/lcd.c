...
//This is still a mystery to me, but I had to invert the display and reverse the RGB setting to get the colors to work 
//correctly in this particular display. If you have trouble, consider removing this command. 
...
/*In setting up the memory access controller, I elected to use the “mirror x” and “mirror y” commands to reorient 
the x and y axes to agree with the silk screen lettering on the Olimex board. If you want the default orientation, 
send the data byte 0x08 instead. Finally, I had to reverse the RGB color setting to get the color information to 
work properly. You may want to experiment with this setting. 
*/ 
...
//I found that setting the contrast varies from display to display. You may want to try several different contrast 
//data values and observe the results on your display. 
...
//Now that the display is initialized properly, we can turn on the display and we’re ready to start producing 
...
void LCDSetLine(int x0, int y0, int x1, int y1, int color) {
        ...
        //    built into the controller. By defining a drawing box, the memory can
...
//         LCDWritebmp.c
//
//     Writes the selected screen area from a graphic array in memory
//
//     Inputs:  picture in memory (as array)
//
//  *****************************************************************************
void LCDWrite130x130bmp(int left_pos, int bottom_pos, int width, int height, unsigned char graph[])
{ //width must be even
 long    j;    // loop counter
// char byte_1, byte_2;
...
*/
 ...
 //WriteSpiCommand(DISOFF);
  ...
  // Column address set  (command 0x2A)
 WriteSpiCommand(CASET);
 WriteSpiData(left_pos);
 WriteSpiData(left_pos+width);

  // Page address set  (command 0x2B)
 WriteSpiCommand(PASET);
 WriteSpiData(bottom_pos);
 WriteSpiData(bottom_pos+width);
 ...
 WriteSpiCommand(RAMWR);
  ...
  for(j = 0; j < ((width>>1)*height); j++) 
	{
		//byte_1=(graph[j*2] & 0x7) | ((graph[j*2] & 0x38) << 1);
		//byte_2=(graph[j*2+1] & 0x7) | ((graph[j*2] & 0x38) << 1);
		
		WriteSpiData( graph[j*2]           );
 }
/*
...
*/
 ...
 //WriteSpiCommand(DISON);
        ...
        //  *********************************************************************************
        //    Font tables for Nokia 6610 LCD Display Driver
        //
